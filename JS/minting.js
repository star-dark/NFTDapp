...
async function connect() {
    ...
    if (klaytn.networkVersion === 8217) {
        ...
        alert("ERROR: 이더리움 네트워크로 연결되지 않았습니다!");
        return;
    }
    account = accounts[0];
    caver.ether.getBalance(account)
        .then(function (balance) {
            document.getElementById("myWallet").innerHTML = `지갑주소: ${account}`
            document.getElementById("myKlay").innerHTML = `잔액: ${caver.utils.fromPeb(balance, "ETHER")} ETHER`
        });
    await check_status();
}
...
async function check_status() {
    ...
    await myContract.methods.mintingInformation().call()
        .then(function (result) {
            ...
            document.getElementById("mintLimitPerBlock").innerHTML = `트랜잭션당 최대 수량: ${mintLimitPerBlock}`;
            ...
            document.getElementById("mintPrice").innerHTML = `민팅 가격: ${caver.utils.fromPeb(mintPrice, "ETHER")} ETHER`;
        })
        .catch(function (error) {
            console.log(error);
        });
    blockNumber = await caver.ether.getBlockNumber();
    document.getElementById("blockNubmer").innerHTML = "현재 블록: #" + blockNumber;
    cntBlockNumber();
}

async function publicMint() {
    if (ether.networkVersion === 8217) {
        console.log("메인넷");
    } else if (klaytn.networkVersion === 1001) {
        console.log("테스트넷");
    } else {
    ...
    const myContract = new caver.ether.Contract(ABI, CONTRACTADDRESS);
